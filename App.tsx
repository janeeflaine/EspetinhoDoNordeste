  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('Store Config Updated:', payload.new);
          setIsStoreOpen(payload.new.is_open);
          // If message ID changed, we might need to refetch message, but usually is_open is what triggers immediate block
          // Ideally we fetch the full config + message again or just update state if payload has enough info
          // Payload only has config columns. If message_id changed, we need new message.
          fetchStoreStatus();
        }
      )
      .subscribe();
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          // Config doesn't exist yet, assume open or init
          console.warn("Store config not found, assuming open.");
          setIsStoreOpen(true);
        } else {
          console.error('Error fetching store status:', error);
          // Fallback to open to not block business on error? Or closed to be safe?
          // Let's fallback to OPEN but log error.
        }
      } else if (data) {
        setIsStoreOpen(data.is_open);
        // @ts-ignore - Supabase types join handling can be tricky to infer perfectly without generated types
